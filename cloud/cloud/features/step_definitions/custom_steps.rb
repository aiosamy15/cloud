...
#puts ' executing precondition signup'
  ...
  # added for new bay account deletion 
   if (ENV["delnewbay"]=="no")
  user_page.logged_in?
  else 
  confirmemail
...
Given /^I clear data for cloud app$/ do
...
performAction('wait_for_view_by_id', 'buttonnext_safestorage_splashscreen')
                   ...
                   # added for new bay account deletion 
def confirmemail
Watir::Wait.until {$browser.text_field(:name => 'confirmEmail').exist?}
$browser.text_field(:name => 'email').set ENV["username"]
$browser.text_field(:name => 'confirmEmail').set ENV["username"]
Watir::Wait.until {$browser.link(:id => 'changeEmailButton').exist?}
$browser.link(:id =>'changeEmailButton').click
Watir::Wait.until {$browser.div(:id => 'paginationLeft').exist?}
assert_equal($browser.span(:id => "menu-settings-account-name").exists? , true)
end
